 ...
 *
 * $Id: dhd_msgbuf.c 669438 2016-11-09 13:11:41Z $
 */
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	if (dhd_dma_buf_alloc(dhd, &dhd->prot->fw_trap_buf, sizeof(uint32))) {
...
dhd_prot_debug_info_print(dhd_pub_t *dhd)
{
	...
	if (dhd->bus->intstatus != (uint32)-1) {
		...
		DHD_ERROR(("Pcie RC Error Status Reg(0x%x) Val=0x%x\n",
			PCIE_CFG_UC_ERR_STS, dhdpcie_rc_config_read(dhd->bus,
			PCIE_CFG_UC_ERR_STS)));
