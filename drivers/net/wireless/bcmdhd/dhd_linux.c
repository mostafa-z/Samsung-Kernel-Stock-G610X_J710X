 ...
 *
 * $Id: dhd_linux.c 669888 2016-11-11 14:35:00Z $
 */
...
/* check the coexistence of DHD_USE_CLMINFO_PARSER and COUNTRY_SINGLE_REGREV */
#if defined(DHD_USE_CLMINFO_PARSER) && defined(COUNTRY_SINGLE_REGREV)
#error "DHD_USE_CLMINFO_PARSER and COUNTRY_SINGLE_REGREV cannot co-exist."
#endif /* DHD_USE_CLMINFO_PARSER && COUNTRY_SINGLE_REGREV */
	...
	struct bcmstrbuf *strbuf, uint32 **hist)
{
	...
	per_cpu_total = (uint32 *)kmalloc(sizeof(uint32) * num_cpus, flags);
	if (!per_cpu_total) {
...
#ifdef WLADPS
#define ADPS_MODE_DEFAULT_PM2 1
int
dhd_preinit_adps_ioctls(dhd_pub_t *dhd, char *iov_buf_smlen)
{
	int ret = BCME_OK, i;
	char iovbuf[WL_EVENTING_MASK_LEN + 12];	/*  Room for "event_msgs" + '\0' + bitvec  */

	wl_adps_params_v1_t adps;

	memset(&adps, 0, ADPS_PARAMS_V1_LEN);

	adps.ver = ADPS_VERSION;
	adps.len = ADPS_PARAMS_V1_LEN;
	adps.mode = ADPS_MODE_DEFAULT_PM2;

	for (i = 1; i <= MAX_BANDS; i++) {
		adps.band = i;
		bcm_mkiovar("adps", (char *)&adps, ADPS_PARAMS_V1_LEN, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
			sizeof(iovbuf), TRUE, 0)) < 0) {
			if (ret == BCME_UNSUPPORTED) {
				...
				DHD_ERROR(("%s Set adps to %d failed  %d\n",
						__FUNCTION__, adps.mode, ret));
				return ret;
			}
		}
	}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (dhd->op_mode & DHD_FLAG_STA_MODE) {
		if ((ret = dhd_preinit_adps_ioctls(dhd, iov_buf)) != BCME_OK) {
			DHD_ERROR(("%s dhd_preinit_adps_ioctls failed %d\n",
					__FUNCTION__, ret));
		}
	}
#endif /* WLADPS */
...
dhd_convert_memdump_type_to_str(uint32 type, char *buf)
{
	...
	switch (type) {
		...
		case DUMP_TYPE_TRANS_ID_MISMATCH:
			...
			break;
		case DUMP_TYPE_HANG_ON_P2P_IFACE_DEL_FAIL:
			type_str = "HANG_P2P_IFACE_DEL_FAIL";
			break;
		default:
