 ...
 *
 * $Id: dhd_pcie.c 669480 2016-11-09 17:46:49Z $
 */
...
int
dhdpcie_config_restore(dhd_bus_t *bus)
{
	uint32 i;
	osl_t *osh = bus->osh;
	...
	OSL_PCI_WRITE_CONFIG(osh, PCIECFGREG_PM_CSR, sizeof(uint32), bus->saved_config.pmcsr);
...
dhd_bus_device_tx_stuck_scan(dhd_bus_t *bus)
{
	...
	for (item = dll_tail_p(&bus->flowring_active_list);
		...
		ring_empty = dhd_prot_is_ring_empty(bus->dhd, flow_ring_node->prot_info);
...
{
	unsigned long flags;
#if defined(PCIE_OOB) || defined(PCIE_INB_DW) || defined(DHD_DEBUG)
	dhd_bus_t *bus;
	bus = dhd->bus;
#endif /* defined(PCIE_OOB)||defined(PCIE_INB_DW)||defined(DHD_DEBUG) */
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_SVAL(IOV_CONTROL_SIGNAL):
		{
			bcmerror = dhd_control_signal(arg, TRUE);
			break;
		}

	case IOV_GVAL(IOV_CONTROL_SIGNAL):
		{
			bcmerror = dhd_control_signal(params, FALSE);
			break;
		}
#endif
	...
	if (state) {
		...
		if (bus->wait_for_d3_ack) {
			...
			if (active) {
				...
				/* Clear the intstatus PCIMailBoxInt Before sending Host Ready */
		...
		} else if (timeleft == 0 && (!dhd_query_bus_erros(bus->dhd))) {
