                        ...
                        tANI_U16 *pDstDataLen, tSirModifyIE *pModifyIE)
{
    ...
    uint8_t *pIe = NULL;
    uint8_t ss_oui_type[3] = "\x00\x16\x32";
     ...
     * shall be updated.
     */
    pIe = cfg_get_vendor_ie_ptr_from_oui(pMac, ss_oui_type, sizeof(ss_oui_type),
                 *pDstData_buff, *pDstDataLen);

    if (pIe) {
        VOS_ASSERT((pIe[1] + 2) == pModifyIE->ieBufferlength);
        vos_mem_copy(pIe, pModifyIE->pIEBuffer, pModifyIE->ieBufferlength);
    } else {
