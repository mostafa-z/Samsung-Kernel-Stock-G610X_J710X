...
void hdd_send_hang_event(int event)
{
    ...
    cfg80211_disconnected(pAdapter->dev, event, NULL, 0, GFP_KERNEL);
}
#endif /* 20160326 AI 4 */
...
void hdd_dump_concurrency_info(hdd_context_t *pHddCtx)
{
   ...
   bool mcc_mode = FALSE;
   v_BOOL_t isConnected = VOS_FALSE;
...
#ifdef CONFIG_SEC
    hddLog(LOGE, "%s: isConnected %d", __func__, isConnected);
    if (isConnected) {
...
#define HDD_WLAN_START_WAIT_TIME VOS_WDA_TIMEOUT + 300000
