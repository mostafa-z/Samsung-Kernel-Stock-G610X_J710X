...
void csrPurgeOldScanResults(tpAniSirGlobal pMac)
{
    ...
    while( pEntry )
    {
        ...
        smsLog(pMac, LOG1, FL("old entry time (%d) for BSSID" MAC_ADDRESS_STR ),
                (pResult->Result.BssDescriptor.nReceivedTime),
                MAC_ADDR_ARRAY(pResult->Result.BssDescriptor.bssId));
        if((curTime - pResult->Result.BssDescriptor.nReceivedTime) > oldest_entry)
        {
            smsLog(pMac, LOG1, FL("Old Bss identified"));
            oldest_entry =  curTime - pResult->Result.BssDescriptor.nReceivedTime;
            oldest_bss = pResult;
        }
        pEntry = tmpEntry;
    }
    if (oldest_bss)
    {
        ...
        if( csrLLRemoveEntry(&pMac->scan.scanResultList, &oldest_bss->Link, LL_ACCESS_NOLOCK) )
        {
            smsLog(pMac, LOG1, FL(" Current time (%d) for BSSID to be removed" MAC_ADDRESS_STR ),
    ...
    for(i = 0; i < pScanChnInfo->numChn; i++)
    {
        ...
        while( pEntry )
        {
            ...
            if(pResult->Result.BssDescriptor.channelId == pChnInfo->channelId)
            {
                ...
                smsLog(pMac, LOG1, FL("AgingCount=%d"), pResult->AgingCount);
            }
            pEntry = tmpEntry;
        }
    }
...
static void csrScanResultCfgAgingTimerHandler(void *pv)
{
    ...
    csrLLLock(&pMac->scan.scanResultList);
    pEntry = csrLLPeekHead( &pMac->scan.scanResultList, LL_ACCESS_NOLOCK );
    smsLog(pMac, LOGE, FL(" Ageout time=%d"),ageOutTime);
    while( pEntry )
    {
        tmpEntry = csrLLNext(&pMac->scan.scanResultList, pEntry,
                                      LL_ACCESS_NOLOCK);
        ...
        smsLog(pMac, LOGE, FL("Current time (%d) for BSSID" MAC_ADDRESS_STR ),
                (curTime - pResult->Result.BssDescriptor.nReceivedTime),
                MAC_ADDR_ARRAY(pResult->Result.BssDescriptor.bssId));
