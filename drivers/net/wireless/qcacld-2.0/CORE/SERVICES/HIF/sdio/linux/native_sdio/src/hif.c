...
#define HIF_WAIT_FOR_POLL_TIME_MS      1000
...
void HIF_PollHandler(struct work_struct *work)
{
    A_STATUS status;
    A_UINT8 host_int_status;
    HIF_DEVICE *device = container_of(to_delayed_work(work),
                                      HIF_DEVICE, poll_work);
    ...
    printk("%s %d\n", __func__, cnss_wlan_get_pending_irq());
    if (status) {
        printk("%s read host int status failed %d\n", __func__, status);
        return;
    }

    if (host_int_status & 0x3) {
        printk("pending data in Mbox, do polling\n");
        cnss_wlan_irq_poll();
    }
#define HIF_MAX_POLL_COUNT 2
    if (poll_count++ < HIF_MAX_POLL_COUNT) {
        HIFStartPollwork(device, 2000);
    }
...
#ifdef CONFIG_CNSS
        cnss_init_delayed_work(&device->poll_work, HIF_PollHandler);
#else
        INIT_DELAYED_WORK(&device->poll_work, HIF_PollHandler);
#endif
...
void
HIFStartPollwork(HIF_DEVICE *device, u_int16_t timeout)
{
    if (!device)
        return;

    timeout = (timeout > 0) ? timeout : HIF_WAIT_FOR_POLL_TIME_MS;
    schedule_delayed_work(&device->poll_work,
                          msecs_to_jiffies(timeout));
}

void
HIFStopPollwork(HIF_DEVICE *device)
{
    if (!device)
        return;

    cancel_delayed_work_sync(&device->poll_work);
}
