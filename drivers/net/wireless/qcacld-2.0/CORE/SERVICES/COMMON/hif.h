...
void HIFStartPollwork(HIF_DEVICE *device, u_int16_t timeout);

void HIFStopPollwork(HIF_DEVICE *device);
...
int hif_pm_runtime_prevent_suspend_timeout(void *ol_sc, void *data,
						unsigned int delay);
#else
extern void HIFSetTargetSleep(HIF_DEVICE *hif_device, A_BOOL sleep_ok, A_BOOL wait_for_it);
