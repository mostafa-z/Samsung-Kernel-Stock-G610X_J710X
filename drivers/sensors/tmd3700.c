...
#define IR_Coef1                (38)
#define CT_Coef1                (3193)
#define CT_Offset1              (1950)
...
#endif
static void tmd3700_interrupt_clear(struct taos_data *taos);
...
static int opt_i2c_read(struct taos_data *taos, u8 reg)
{
	int ret;
...
static int proximity_get_adc(struct taos_data *taos)
{
	...
	adc = opt_i2c_read(taos, PRX_DATA_HIGH);
...
static int taos_proximity_get_threshold(struct taos_data *taos, u8 buf)
{
	...
	threshold = opt_i2c_read(taos, buf);
	if (threshold < 0)
		SENSOR_ERR("i2c read failed (%d)\n", threshold);
...
static void taos_thresh_set(struct taos_data *taos)
{
	...
	ret = opt_i2c_write(taos, PRX_MINTHRESH, &prox_int_thresh[0]);
	if (ret < 0)
		SENSOR_ERR("opt_i2c_write failed, err = %d\n", ret);

	ret = opt_i2c_write(taos, PRX_MAXTHRESH, &prox_int_thresh[1]);
	if (ret < 0)
		SENSOR_ERR("opt_i2c_write failed, err = %d\n", ret);
}

static int taos_chip_init(struct taos_data *taos)
{
	u8 temp_val;
	int ret = 0;
	/************************************/
	/* Apply factory trim for proximity */
	u8 ppulse_len;
	u8 ppulse;
	u8 pre_offset;
	u8 pgain = 0;
	...
	SENSOR_INFO("dgf:%d, cctcoef:%d, cctoff:%d, r:%d, g:%d, b:%d\n",
		taos->pdata->dgf, taos->pdata->cct_coef,
		taos->pdata->cct_offset, taos->pdata->coef_r,
		taos->pdata->coef_g, taos->pdata->coef_b);

	/* reset interrupt flags */
	temp_val = int_read_clear;
	ret = opt_i2c_write(taos, CFG3, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write cfg3 reg(%d)\n", ret);

	/* ALS init */
	temp_val = 0x7f; /* one-shot autozero */
	ret = opt_i2c_write(taos, AZ_CONFIG, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write auto zero cfg reg(%d)\n", ret);

	temp_val = taos->pdata->als_gain; /* ALS GAIN Default 0x02(x16) */
	ret = opt_i2c_write(taos, CFG1, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write als gain ctrl reg(%d)\n", ret);

	temp_val = taos->pdata->als_time;
	ret = opt_i2c_write(taos, ALS_TIME, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write als time reg(%d)\n", ret);

	SENSOR_INFO("default again reg val = %d\n", taos->pdata->als_gain);

	/* PROXIMITY init */
	temp_val = 0x10; /* proximity upper 4bit | als lower 4bit :  proximity cycle -any, als cycle  0 every time */
	ret = opt_i2c_write(taos, PPERS, &temp_val);
	if (ret < 0)
	...
	pre_offset = opt_i2c_read(taos, FACTORYTRIM);
	if (pre_offset < 0)
		SENSOR_ERR("failed to read factory trim(%d)\n", pre_offset);
	/* 2 . Read Factory trim Sign */
	pre_offset_sign = opt_i2c_read(taos, FACTORYTRIMSIGN);
	if (pre_offset_sign < 0)
		SENSOR_ERR("failed to read factory trim sign(%d)\n",
			pre_offset_sign);
	...
	if (ret < 0)
		SENSOR_ERR("failed to write proximity offsetL reg(%d)\n", ret);

	temp_val = pre_offset_sign;
	ret = opt_i2c_write(taos, POFFSET_H, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write proximity offsetH reg(%d)\n", ret);
	/************************************/
	/* END                             **/
...
static int taos_chip_off(struct taos_data *taos)
{
	...
	ret = opt_i2c_write(taos, INTENAB, &reg_cntrl);
	if (ret < 0) {
...
static int taos_als_enable(struct taos_data *taos)
{
	u8 temp_val;
	int ret;
	...
	ret = opt_i2c_write(taos, INTENAB, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write int enable reg(%d)\n", ret);

	/* wen|pen|aen|pon, als , poweron  enable */
	temp_val = pon | aen;
	ret = opt_i2c_write(taos, ENABLE, &temp_val);
	if (ret < 0)
...
static int taos_prox_enable(struct taos_data *taos)
{
	u8 temp_val;
	int ret;
	...
	temp_val = 0x90; /* pagin 4x , pgldrive 102mA */
	ret = opt_i2c_write(taos, PGCFG1, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write prox pulse reg(%d)\n", ret);

	temp_val = pien; /* asein|psien|pien|aien|cien|0|0|0 */
	ret = opt_i2c_write(taos, INTENAB, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write int enable reg(%d)\n", ret);

	/* wen|pen|aen|pon, Proximity, poweron  enable */
	temp_val = pen | pon | wen;
	ret = opt_i2c_write(taos, ENABLE, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write enable state reg(%d)\n", ret);
...
static int taos_als_prox_enable(struct taos_data *taos)
{
	int ret;
	u8 temp_val;
	...
	/* wen|pen|aen|pon, Proximity , als , poweron  enable */
	temp_val = pen | pon | aen | wen;
	ret = opt_i2c_write(taos, ENABLE, &temp_val);
	if (ret < 0)
		SENSOR_ERR("failed to write enable state reg(%d)\n", ret);
...
/*!
******************************************************************************
*   taos_set_op_mode  : Set operating mode (prox, als etc)
*
*   \param    op_mode :   proximity, als, etc
*   \param    state :   0 -> unset , 1 -> set
*
*   \return 0 : success
******************************************************************************
*/
static u16 device_master_state;

static int taos_set_op_mode(u16 op_mode, u8 state, struct taos_data *taos)
{
	int ret = 0;
	u16 last_state;

	/* Determine what is already on or off */
	last_state = device_master_state;
	if (state)
		device_master_state |= (op_mode << 0);
	else
		device_master_state &= ~(op_mode << 0);

	switch (device_master_state) {
	case mode_off:
		SENSOR_INFO("#> mode_off\n");
		ret = taos_chip_off(taos);
		if (ret < 0)
			SENSOR_ERR("failed to chip off(%d)\n", ret);
		break;
	case mode_als:
		SENSOR_INFO("#> mode_als\n");
		ret = taos_als_enable(taos);
		if (ret < 0)
			SENSOR_ERR("failed to als enable(%d)\n", ret);
		break;
	case mode_prox:
		SENSOR_INFO("#> mode_prox\n");
		ret = taos_prox_enable(taos);
		if (ret < 0)
			SENSOR_ERR("failed to prox enable(%d)\n", ret);
		break;
	case mode_als_prox: /* already als on -> prox on || already prox on -> als on */
		SENSOR_INFO("#> mode_als_prox\n");
		ret = taos_als_prox_enable(taos);
		if (ret < 0)
...
static int taos_get_lux(struct taos_data *taos)
{
	...
	temp_gain = i2c_smbus_read_word_data(taos->i2c_client, CFG1);
...
*/
	...
	if (gain == 1 && clrdata < 25) {
		...
		ret = opt_i2c_write(taos, CFG1, &reg_gain);
		if (ret < 0)
			SENSOR_ERR("opt_i2c_write failed, err = %d\n", ret);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !(taos->power_state & LIGHT_ENABLED)) {
		taos->power_state |= LIGHT_ENABLED;
		taos_set_op_mode(mode_als, ON, taos);
		taos_light_enable(taos);
	} else if (!new_value && (taos->power_state & LIGHT_ENABLED)) {
		taos_light_disable(taos);
		taos_set_op_mode(mode_als, OFF, taos);
		taos->power_state &= ~LIGHT_ENABLED;
	}
	mutex_unlock(&taos->power_lock);
	return size;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !(taos->power_state & PROXIMITY_ENABLED)) {
		...
		taos_set_op_mode(mode_prox, ON, taos);
		...
		taos_set_op_mode(mode_prox, OFF, taos);
...
#ifdef CONFIG_OF
static struct of_device_id tm3700_match_table[] = {
	{ .compatible = "taos,tmd3700",},
};
#else
#define tm3700_match_table NULL
#endif
...
static struct i2c_driver taos_i2c_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &taos_pm_ops,
		.of_match_table = tm3700_match_table,
	},
