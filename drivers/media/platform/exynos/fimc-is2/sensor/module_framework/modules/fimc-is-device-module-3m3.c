...
static struct fimc_is_sensor_cfg config_module_3m3[] = {
	...
	/* 2072x1552@30fps */
	FIMC_IS_SENSOR_CFG(2072, 1552, 30, 30, 2, CSI_DATA_LANES_4),
	/* 2072x1552@15fps */
	FIMC_IS_SENSOR_CFG(2072, 1552, 15, 30, 3, CSI_DATA_LANES_4),
	/* 2072x1552@7fps */
	FIMC_IS_SENSOR_CFG(2072, 1552, 7, 30, 4, CSI_DATA_LANES_4),
	/* 2072x1162@30fps */
	FIMC_IS_SENSOR_CFG(2072, 1162, 30, 24, 5, CSI_DATA_LANES_4),
	/* 1008x754@120fps */
	...
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	if (!gpio_is_valid(gpio_camio_1p8_en)) {
		warn("%s failed to get gpio_camio_1p8_en\n",__func__);
	} else {
	...
	if (!gpio_is_valid(gpio_cam_1p05_en)) {
		warn("%s failed to get gpio_cam_1p05_en\n", __func__);
	} else {
		gpio_request_one(gpio_cam_1p05_en, GPIOF_OUT_INIT_LOW, "CAM_GPIO_OUTPUT_LOW");
		gpio_free(gpio_cam_1p05_en);
	}

	gpio_cam_a2p8_en = of_get_named_gpio(dnode, "gpio_cam_a2p8_en", 0);
	if (!gpio_is_valid(gpio_cam_a2p8_en)) {
		warn("%s failed to get gpio_cam_a2p8_en\n",__func__);
	} else {
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_cam_1p05_en, "cam_1p05_en", PIN_OUTPUT, 1, 0);
	if (gpio_is_valid(gpio_cam_a2p8_en)) {
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "pin", PIN_FUNCTION, 1, 5000);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_cam_1p05_en, "cam_1p05_en", PIN_OUTPUT, 0, 0);
	if (gpio_is_valid(gpio_cam_a2p8_en)) {
