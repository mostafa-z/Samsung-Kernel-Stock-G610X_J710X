...
#ifdef CONFIG_TOUCHKEY_GRIP
/*
* Sysfs print grip intensity
*/
static ssize_t mip4_tk_cmd_grip_image(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mip4_tk_info *info = dev_get_drvdata(dev);
	int type = 0;
	int retry = 10;
...
* Show grip threshold
*/
static ssize_t mip4_tk_cmd_grip_threshold(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mip4_tk_info *info = dev_get_drvdata(dev);
	u8 wbuf[2];
	u8 rbuf[2];
	...
	wbuf[0] = MIP_R0_INFO;
	wbuf[1] = MIP_R1_INFO_CONTACT_THD_GRIP;
	if (mip4_tk_i2c_read(info, wbuf, 2, rbuf, 1)) {
		goto error;
	}

	threshold = (int)rbuf[0];

	input_dbg(true,&info->client->dev, "%s - grip threshold [%d]\n", __func__, threshold);
	return snprintf(buf, PAGE_SIZE, "%d\n", threshold);
...
/*
* Sysfs show grip event state
*/
static ssize_t mip4_tk_cmd_grip_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mip4_tk_info *info = dev_get_drvdata(dev);
	u8 data[255];
	int ret;
	...
	input_info(true,&info->client->dev, "%s - grip state[%d]\n", __func__, info->grip_state);
	sprintf(data, "%s : %d\n", attr->attr.name, info->grip_state);
...
static DEVICE_ATTR(grip, S_IRUGO, mip4_tk_cmd_grip_image, NULL);
static DEVICE_ATTR(grip_raw, S_IRUGO, mip4_tk_cmd_grip_image, NULL);
static DEVICE_ATTR(grip_cp, S_IRUGO, mip4_tk_cmd_grip_test, NULL);
static DEVICE_ATTR(grip_threshold, S_IRUGO, mip4_tk_cmd_grip_threshold, NULL);
static DEVICE_ATTR(grip_recal, S_IRUGO | S_IWUSR | S_IWGRP, mip4_tk_cmd_recal_show, mip4_tk_cmd_recal_store);
static DEVICE_ATTR(grip_enable, S_IRUGO | S_IWUSR | S_IWGRP, mip4_tk_cmd_enable_show, mip4_tk_cmd_enable_store);
static DEVICE_ATTR(grip_state, S_IRUGO, mip4_tk_cmd_grip_state, NULL);
#endif
#ifdef MIP_USE_LED
...
static struct attribute *mip_cmd_key_attr[] = {
	...
	&dev_attr_grip.attr,
	&dev_attr_grip_raw.attr,
	&dev_attr_grip_cp.attr,
	&dev_attr_grip_threshold.attr,
	&dev_attr_grip_state.attr,
	&dev_attr_grip_recal.attr,
