				...
				char *buf)
{
	...
	struct i2c_client *i2c = muic_data->i2c;
	int val = 0; 
	u8 ret, vbadc, afc_ctrl;


	/* 1. Set AFC_EN, VB_ADC_EN to true for PD charger */
	...
	pr_info("%s:%s vbadc:0x%x, afc_ctrl:0x%x\n", MUIC_DEV_NAME, __func__, vbadc, afc_ctrl);
