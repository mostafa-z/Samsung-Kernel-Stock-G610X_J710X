...
static void dsim_panel_aid_ctrl(struct lcd_info *lcd, u8 force)
{
	...
	if (lcd->aor_table[lcd->bl] == NULL) {
		dev_err(&lcd->ld->dev, "%s: failed to get aid value\n", __func__);
		goto exit;
	}

	aor_value.aor_1 = lcd->aor_table[lcd->bl][LDI_OFFSET_AOR_1];
	aor_value.aor_2 = lcd->aor_table[lcd->bl][LDI_OFFSET_AOR_2];

	DSI_WRITE(lcd->aor_table[lcd->bl], AID_CMD_CNT);
...
static int dsim_panel_set_hbm(struct lcd_info *lcd, int force)
{
	...
	if (force || lcd->current_hbm != lcd->hbm_table[level][LDI_OFFSET_HBM]) {
		...
		if (level)
			DSI_WRITE(lcd->hbm_gamma, ARRAY_SIZE(lcd->hbm_gamma));
...
static int s6e8aa5x01_init_hbm_gamma(struct lcd_info *lcd)
{
	...
	memcpy(lcd->hbm_gamma, SEQ_GAMMA_HBM_SET, HBM_GAMMA_CMD_CNT);

	/* hbm gamma is written on 0xC8 [34~39] and [73~87] */
	DSI_WRITE(SEQ_MTP_READ_HBM_GP_1, ARRAY_SIZE(SEQ_MTP_READ_HBM_GP_1));

	ret = s6e8aa5x01_read_info(lcd, LDI_REG_MTP, LDI_LEN_HBM_GAMMA_1, &lcd->hbm_gamma[2]);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_MTP, LDI_LEN_HBM_GAMMA_2, &lcd->hbm_gamma[8]);
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
		goto exit;
	}
