...
#define COD3026X_WATER_DET_THRESHOLD_MIN_FIRST 250
#define COD3026X_WATER_DET_THRESHOLD_MIN 40
...
//			jd->jack_det = jackdet ? false : true;
			...
			if(cod3026x->use_det_gdet_adc_mode){
				if(jd->jack_det == true){
					queue_delayed_work(cod3026x->water_det_adc_wq,
							&cod3026x->water_det_adc_work, msecs_to_jiffies(100));
				}
			}
			/* mic detection delay */
			if (jd->jack_det) {
